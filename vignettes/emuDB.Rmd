...
*NOTE: If there are actual instances of annotation items present in the emuDB removing the levelDefinition will not be possible. These would first have to be removed.*
...
*NOTE: The legalLabels field is not obligatory. If not set the attributeDefinition can contain any label.*
...
*NOTE: It is also possible to define label groups for the entire DB. For more information on this see the ??? section of this document.*
...
As the names imply these links specify the permitted relationships between the items of one level to another. The structure of the hierarchy of the 'ae' demo database that comes with the emuR package can be seen below.
...
*NOTE: due to the ability of the `get_trackdata()` function to utilize any of the signal processing functions of the wrassp package to calculate the necessary values on-the-fly it is quite often not necessary to define ssffTracks for tracks produced by the wrassp package. For complimentary data as well as when data that will be manually manipulated (e.g. manual formant corrections) this is still a feasible and necessary option. Also, if you wish to display SSFF data in the EMU-webApp it is necessary to pre-calculate the ssffTracks as the web application can not perform on-the-fly calculations.*
...
Before we can get started with the process of manually annotating our speech database we have to configure our 'fromScratchDB' to contain information about how the database is to be displayed by the EMU-webApp. The EMU-webApp provides a mechanism to view your database from different perspectives. These perspectives have to be defined bla bla
