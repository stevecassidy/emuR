...
```{r}
# gernerate path to the emtpy fromScratchDB created above
...
If we wish to change the name/type of the level we have just added to the database we can invoke (NOTE: If there are actual instances of annotation items present in the emuDB this will not be possible. These would first have to be removed.):   
...
```{r}
# add
add_levelDefinition(dbName, name = 'Word', type = 'ITEM')
# list
list_levelDefinitions(dbName)
```

Finally we could remove one of the levelDefinitions with the function `remove_levelDefinition()`, which we will not invoke here as we will use the 'phonetic' level below. 
...
```{r}
# list
list_attributeDefinitions(dbName, 'phonetic')
```
...
```{r}
# add
add_attributeDefinition(dbName, 'phonetic', newName = 'SAMPA')
# list
list_attributeDefinitions(dbName, 'phonetic')
```
...
TODO write about legal labels field
...
Up until now we have defined the structure of our database. An essential part that is missing is of course the recordings that we wish to analyse. To import audio files, refered to as media files in the context of an emuDB, into the database one simply has to do the following:
...
import_mediaFiles(dbName, dir = fp, targetSessionName = 'wrassp', verbose = F)
# list session
...
We have now added a new session containing new bundles to the database that adhere to the structure we have specified above. Note that the levels in the annotation files (`_annot.json`) that where created during the import are still empty. These will have to be created manually using the EMU-webApp. To list the files that are part of the emuDB simply:
...
wavFilePaths = list.files(dbPath, pattern = "wav$", full.names = T, recursive = T)
...
```{r}
#################################
# add ssffTrackDefinition

# add track without calculating SSFF files
# add_ssffTrackDefinition('ae', name = 'coolName', columnName = 'fm', fileExtension = 'fms')

# or alternatively add track + calculate SSFF files for entire emuDB
# add_ssffTrackDefinition('ae', name = 'coolName', columnName = 'fm', fileExtension = 'fms', onTheFlyFunctionName = 'forest')

#################################
# calculate SSFF files using wrassp

# list wav files 
# fp=list.files('/Users/raphaelwinkelmann/Developer/emuR/inst/extdata/emu/DBs/ae', recursive = T, full.names = T, pattern = 'wav$')

# calculate formants (NOTE: no ssffTrackDefinitions entry generated! Has to be done manually or using add_ssffTrackDefinition())
# forest(fp)
```
