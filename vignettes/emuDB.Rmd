...
- the database folder has to have the same name as the `_DBconfig.json` prefix and may optionally have the suffix `_emuDB`. Note that the `_DBconfig.json` contains the field `name` which specifies the name of the emuDB. This also has to have the same name as the folder and `_DBconfig.json` prefix.
...
There are multiple ways of creating emuDBs. The two main strategies are to either convert existing databases or file collections to the new format or to create new databases from scratch. As the `emuR\_intro` vignette, that can be accessed by invoking this command: `vignette("emuR_intro", package="emuR")`, already describes how existing databases can be converted, only the latter of both strategies is described below.
...
- `"EMUwebAppConfig"` specifies the configuration options intended for the EMU-webApp, i.e. how the database is to be displayed. This field can contain all the configurations options that are specified in the EMU-webApp's configuration schema (*https://github.com/IPS-LMU/EMU-webApp/blob/master/dist/schemaFiles/emuwebappConfigSchema.json*). The `"EMUwebAppConfig"` contains the following fields:
    ...
    - `"perspectives"` array containing perspective configurations. Each `"perspective"` consists of:
        ...
        - `"twoDimCanvases"` configuration options for the 2D canvas
            ...
            - `"twoDimDrawingDefinitions"` array containing two dimentional drawing definitions. Each two dimensional drawing definition consist of: 
                ...
                - `"connectLines"` array specifying which of the dots specified in the dots definition array should be connected with a line
    ...
    - `"restrictions"`
        ...
        - `"bundleFinishedEditing"` boolean value specifying whether to allow the annotator to mark when she / he has finished annotating a bundle. A finished editing toggle button will show up int the bundle list side bar for each bundle if this is set to true. Note that the server has to support saving these comments which the `serve()` function of the `emuR` package doesn't.
    ...
    - `"activeButtons"` specifications of which top-/bottom-menu buttons should be active / displayed by the EMU-webApp
        ...
        - `"saveBundle"` boolean value specifying whether to show save button in bundle list side bar for each bundle
        - `"openMenu"` boolean value specifying whether open bundle list side bar button is displayed
        - `"showHierarchy"` boolean value specifying whether to show "show hierarchy"" button
        - `"editEMUwebAppConfig"` boolean value specifying whether to show "edit EMUwebAppConfig" button
    - `"demoDBs"` array of stings specifying which demoDBs to display in the `open demo` dropdown menu. Currently availale demo databases are `["ae", "ema", "epgdorsal"]`
