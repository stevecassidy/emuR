...
unlink(file.path(tempdir(),'myTGcolDB_emuDB'), recursive = T)
```
...
- `"EMUwebAppConfig"` specifies the configuration options intended for the EMU-webApp, i.e. how the database is to be displayed. This field can contain all the configurations options that are specified in the EMU-webApp's configuration schema (*https://raw.githubusercontent.com/IPS-LMU/EMU-webApp/master/dist/schemaFiles/emuwebappConfigSchema.json*). The `"EMUwebAppConfig"` contains the following fields:
...
- `"demoDBs"` array of stings specifying which demoDBs to display in the `open demo` dropdown menu. Currently available demo databases are `["ae", "ema", "epgdorsal"]`
