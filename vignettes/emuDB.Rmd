...
Before we can get started with the arduous process of manually annotating our speech database we have to configure our 'fromScratchDB' to contain information about how the database is to be displayed by the EMU-webApp. The EMU-webApp subdivides different ways to look at an emuDB into so called perspectives. These switchable perspectives contain information on what levels are displayed, which ssffTracks are drawn and so on. Let us list the current perspectives our our database:
...
As you can see there is already a perspective available called 'default' which was automatically added to the emuDB during the import of our mediaFiles, that currently only displays the oscillogram ("OSCI") followed by the spectrogram ("SPEC"). "OSCI" and "SPEC" can be viewed as predefined ssffTracks that are always available to the EMU-webApp.
Using the `add/remove_perspective()` functions we could now add and remove as many additional perspectives to the database as we like. For now let us just stick to the 'default' perspective and add the order in which we would like to display our levels.
...
We only added the "Phonetic" and not the "Word" level to be displayed in the "default" perspective as only levels of the type "SEGMENT" or "EVENT" are allowed to be displayed. "ITEM" levels can be viewed by clicking the "showHierarchy" button in the top menu bar of the EMU-webApp. As the final configuration step let us also add the ssffTracks we defined above to the "default" perspective:
...
*NOTE: the EMU-webApp is highly configurable and only a small subset of the configuration options are available through the emuR package. In most cases simply manually editing the _DBconfig.json BLA BLA*
