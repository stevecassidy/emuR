...
produce in EQL2 please see the `EQL` vignette.

The `query()` function returns an object with the class `emuRsegs` that is a superclass of the well known `data.frame`. We can now use the information
in this object to calculate the mean durations of these segments:
...
## 3.) What words do the phonetic SEGMENTs that carry the labels s, z, S or Z occur in an what is their Phonetic context?

As we have done before, let us query the `emuDB` for the SEGMENTs we are intersted in:
...
see that the 'Words' level has multiple `attributeDefintions` which indicates that each annotational unit in the `Words` level has muliple
parallel labels defined for it. So let us try the `attributeDefintion` called 'Text'.
...
**NOTE: this two step process can also be completed in a single hierarchical query. See the EQL vignette for more details.**

Now that we have answered the first part of the question let us now look at the left and right context of the extracted sibillants by
using the `requery_seq()` function.

```{r}
# get left context by offsetting one annotational unit to the left
#left = requery_seq(sibil, offset = -1)

#head(left)

# get right context by offsetting one annotational unit to the right
#right = requery_seq(sibil, offset = 1)
...
## 4.) Do the sibilants Phonetic SEGMENTs that carry the labels s; z; S or Z in the 'ae' emuDB differ with respect to their 1st spectral moment?

As we have done before let us query the `emuDB` for the SEGMENTs we are interssted in:  
