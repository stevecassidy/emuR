...
## 3.) What words do the phonetic SEGMENTs that carry the labels s, z, S or Z occur in an what is their perceeding phonetic context?
...
**NOTE: this two step process can also be completed in a single hierarchical query using the dominance operation: ^. See the EQL 
...
Once again let us query the `emuDB` for the SEGMENTs we are interssted in:  
...
```{r results='hide', message=FALSE, warning=FALSE}
sib.tr = get_trackdata(dbName = ae, 
                       seglist = sibil, 
...
### Alternative method using the emuRtrackdata resultType
