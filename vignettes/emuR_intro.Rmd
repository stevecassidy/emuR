...
This will create a new `emuDB` in the folder `tempdir()` called 'myTGcolDB'. The `emuDB` will contain all the tier information from the `.TextGrid` files but will not contain hierarchical information as `.TextGrid` files do not contain any linking information. If you are interested in how to programmatically generate links between the generated `SEGMENT`s and `EVENT`s see the Autobuilding section of the `emuDB` vignette.

```{r}
# remove the newly generated emuDB as we will not be needing it 
# throughout the rest of this vignette
unlink(file.path(tempdir(), "myTGcolDB"), recursive = TRUE)
```
...
# remove the newly generated emuDB as we will not be needing it 
# throughout the rest of this vignette
unlink(file.path(tempdir(), "myBPF-DB"), recursive = TRUE)
```
...
This implicates annotational units from the 'Word' level can somehow be connected to units from the 'Phonetic' level via 
...
The EMU\_SDMS has a fairly unique approach to annotating and the visually inspecting databases, as it 
...
work-flow by walking through a few use cases. Every use case will start off by asking a question about the 'ae' database and will continue by walking
...
As we have done before, let us query the `emuDB` for the segments we are interested in:
...
```{r}
words = requery_hier(sibil, level = "Word")
...
a classification of the words into content words ('C') and function words ('F'). Looking back at the output of `summary_emuDB()` we can 
...
parallel labels defined for it. So let us instead try the `attributeDefintion` called 'Text'.

```{r}
words = requery_hier(sibil, level = "Text")
...
Now that we have answered the first part of the question let us look at the left and right context of the extracted sibilants by
using the `requery_seq()` function.

```{r}
# get left context by off-setting the annotational units in sibil one unit to the left
leftContext = requery_seq(sibil, offset = -1)
...
rightContext = requery_seq(sibil, offset = 1)
```
...
```{r}
rightContext = requery_seq(sibil, 
                           offset = 1, 
...
Once again let us query the `emuDB` for the segments we are interested in (this time using the new regex operand:=~):

```{r}
sibil = query(ae,"Phonetic=~'[szSZ]'")
```

Let us now use `get_trackdata()`, this time to extract the discrete Fourier transform values for our segments:

```{r results='hide', message=FALSE, warning=FALSE}
dftTd = get_trackdata(dbName = ae, 
                      seglist = sibil,
...
To wrap up, let us calculate the averages of these middle trajectories using the `trapply` function:
