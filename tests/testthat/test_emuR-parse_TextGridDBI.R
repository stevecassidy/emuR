...
dbHandle = emuDBhandle(newDbName, basePath, UUIDgenerate(), connectionPath = ":memory:")
parse_TextGridDBI(dbHandle, path2tg, sR, bundle="msajc003", session="0000")

##############################
test_that("correct SEGMENT values are parsed and calculated in SQLite items table", {  
  
  # get Phonetic table
  phoneticTbl <- dbGetQuery(dbHandle$connection, paste0("SELECT * FROM items WHERE db_uuid='", dbHandle$UUID, "' AND level = 'Phonetic'"))
  
  # get labels table
  labelsTbl = dbGetQuery(dbHandle$connection, paste0("SELECT * FROM labels WHERE db_uuid='", dbHandle$UUID,"'"))
  ...
  qRes = dbGetQuery(dbHandle$connection, paste0("SELECT * FROM labels WHERE db_uuid ='", phoneticTbl[2,]$db_uuid, "' ",
  ...
  qRes = dbGetQuery(dbHandle$connection, paste0("SELECT * FROM labels WHERE db_uuid ='", phoneticTbl[18,]$db_uuid, "' ",
  ...
  qRes = dbGetQuery(dbHandle$connection, paste0("SELECT * FROM labels WHERE db_uuid ='", phoneticTbl[35,]$db_uuid, "' ",
...
test_that("correct EVENT values are parsed and calculated in SQLite items table", {
  ...
  # get Tone table
  toneTbl <- dbGetQuery(dbHandle$connection, paste0("SELECT * FROM items WHERE db_uuid='", dbHandle$UUID, "' AND level = 'Tone'"))
  ...
  qRes = dbGetQuery(dbHandle$connection, paste0("SELECT * FROM labels WHERE db_uuid ='", toneTbl[1,]$db_uuid, "' ",
  ...
  qRes = dbGetQuery(dbHandle$connection, paste0("SELECT * FROM labels WHERE db_uuid ='", toneTbl[4,]$db_uuid, "' ",
  ...
  qRes = dbGetQuery(dbHandle$connection, paste0("SELECT * FROM labels WHERE db_uuid ='", toneTbl[7,]$db_uuid, "' ",
...
test_that("SQLite label table has correct values", {
  ...
  # check labelTbl
  labelsTbl = dbGetQuery(dbHandle$connection, paste0("SELECT * FROM labels WHERE db_uuid='", dbHandle$UUID,"' AND name='Phonetic'"))
  expect_equal(paste0(labelsTbl$label, collapse = ''), 'VmVNstH@:frEnzSi:w@zkH@nsId@dbju:dH@f@l')
  labelsTbl = dbGetQuery(dbHandle$connection, paste0("SELECT * FROM labels WHERE db_uuid='", dbHandle$UUID,"' AND name='Tone'"))
