                                 ...
                                 resume = FALSE)
{
  ...
  for (label in c(orthoLabel, canoLabel, mausLabel, sylLabel, canoSylLabel, minniLabel, chunkLabel))
  {
  ...
  if(is.null(chunkLevel))
  {
    ...
    if (nrow(bundles) > 0)
    {
      ...
      {
        bundle = bundles[bundle_idx, "name"]
        session = bundles[bundle_idx, "session"]
        annotates = bas_get_signal_path(handle, session, bundle)
  ...
  if (chunkLevel != transcriptionLevel && !is.null(chunkLevel))
  {
  ...
  autobuild_linkFromTimes(
    handle, sylLevel, mausLevel,
    rewriteAllAnnots = FALSE, convertSuperlevel = TRUE
  )
