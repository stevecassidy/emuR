  ...
  if(prbOpen != -1){
    ...
    if(prbClose == -1){
      ...
      if(funcName == 'Start'){
        ...
        # if(!level2isChild){
        #   # maybe issue this warning earlier if it is rel. for all of the FUNQ queries?
        #   stop(paste0("The first level in the expression: '", qTrim, "' ",
        #               "isn't a parent level of the second level! ",
        #               "Expanding child level sequences for backward compatibility."))
        #   if(funcVal != 1){
        #     stop(paste0("If first level in the expression: '", qTrim, "' ", 
        #                 "isn't a parent of the second level only a function",
        #                 " value of 1 is permitted (i.e. Num(...) = 1)"))
        #   }
        #   # expand left side seqs (child level)
        #   DBI::dbExecute(emuDBhandle$connection, paste0("INSERT INTO items_as_seqs_tmp ",
        #                                                 "SELECT ",
        #                                                 " lrt.db_uuid, ",
        #                                                 " lrt.session, ",
        ...
        #                                                 " 1 AS seq_len, ",
        #                                                 " '", param1, "' AS level ",
        #                                                 "FROM lr_exp_res_tmp AS lrt, ", 
        #                                                 itemsTableName, " AS i1 ",
        #                                                 "WHERE lrt.db_uuid = i1.db_uuid ",
        #                                                 " AND lrt.session = i1.session ",
        #                                                 " AND lrt.bundle = i1.bundle ",
        #                                                 " AND i1.item_id BETWEEN lrt.l_seq_start_id AND lrt.l_seq_end_id ",
        #                                                 #" AND lrt.l_seq_end_id <= i1.item_id ",
        #                                                 # " AND (sit.max_seq_idx - sit.min_seq_idx) + 1 ", # this is always one in this case
        #                                                 # sqlFuncOpr, " ",  
        #                                                 # funcVal, " ",
        #                                                 ""))
        # } else {
...
# attempt of a function to replace query_databaseHier with a "simple"
# CTE version
query_getChildren <- function(emuDBhandle, seglist, childAttributeName) {
  
  testTable = data.frame(staff_id = c(1, 2, 3, 4),
                         first_name = c("Peter", "John", "Michael", "Raphael"),
                         second_name = c("Jones", "Miller", "Smith", "Winkelmann"),
                         manager_id = c(3, 3, 4, NA))
  
  
  DBI::dbWriteTable(emuDBhandle$connection, name = "sales_staffs", testTable)
...
createHierarchyQueryTmpTables <- function(emuDBhandle){
  ...
  hier_left_trapeze_interm_res_tmp = paste0("CREATE TEMP TABLE IF NOT EXISTS hier_left_trapeze_interm_res_tmp (",
                                            ...
                                            " seq_start_seq_idx INTEGER,",
                                            " seq_end_seq_idx INTEGER,",
                                            " db_uuid_child VARCHAR(36),",
                                            " session_child TEXT,",
                                            " bundle_child TEXT,",
                                            " seq_start_id_child INTEGER,",
                                            " seq_end_id_child INTEGER,",
                                            " seq_len_child INTEGER,",
                                            " level_child TEXT,",
                                            " seq_start_seq_idx_child INTEGER,",
                                            " seq_end_seq_idx_child INTEGER,",
                                            " PRIMARY KEY (",
                                            "  db_uuid, ",
                                            "  session, ",
                                            "  bundle, ",
...
query_databaseHier <- function(emuDBhandle, 
                               ...
                               preserveParentLength = F,
                               verbose = F) {
  
  createHierarchyQueryTmpTables(emuDBhandle)
  ...
  for(connectHierPath in connectHierPaths){
    ...
    # depending on what side is the child (== further down in hierarchy) get the correct table name
    if(firstLevelName == connectHierPath[length(connectHierPath)]){
      # left is child
      childSideTableName = paste0("interm_res_items_tmp_", leftTableSuffix)
      # right is parent
      parentSideTableName = paste0("interm_res_items_tmp_", rightTableSuffix)
      # 
      leftIsChild = TRUE
    }else if(secondLevelName == connectHierPath[length(connectHierPath)]){
      # right is child
      childSideTableName = paste0("interm_res_items_tmp_", rightTableSuffix)
      # left is parent
      parentSideTableName = paste0("interm_res_items_tmp_", leftTableSuffix)
      # 
      leftIsChild = FALSE
    }
    ...
    for(i in length(connectHierPath):1){
      if(i == length(connectHierPath)){
        ...
        DBI::dbExecute(emuDBhandle$connection, paste0("INSERT INTO hier_left_trapeze_interm_res_tmp ",
                                                      ...
                                                      " lstn.session, ",
                                                      " lstn.bundle, ",
                                                      " items.item_id AS seq_start_id, ",
                                                      " items.item_id AS seq_end_id, ",
                                                      " 1 AS seq_len, ",
                                                      " items.level, ",
                                                      " items.seq_idx AS seq_start_seq_idx, ",
                                                      " items.seq_idx AS seq_end_seq_idx, ",
                                                      " lstn.db_uuid AS db_uuid_child, ",
                                                      " lstn.session AS session_child, ",
                                                      " lstn.bundle AS bundle_child, ",
                                                      " lstn.seq_start_id AS seq_start_id_child, ",
                                                      " lstn.seq_end_id AS seq_end_id_child, ",
                                                      " lstn.seq_len AS seq_len_child, ",
                                                      " lstn.level AS level_child, ",
                                                      " lstn.seq_start_seq_idx AS seq_start_seq_idx, ",
                                                      " lstn.seq_end_seq_idx AS seq_end_seq_idx ",
                                                      "FROM ", childSideTableName, " AS lstn, ",
                                                      " links, ",
                                                      " items ",
                                                      "WHERE lstn.db_uuid = links.db_uuid ",
                                                      " AND lstn.session = links.session ",
                                                      " AND lstn.bundle = links.bundle ",
                                                      " AND lstn.seq_start_id = links.to_id ", # left side of seq
                                                      " AND links.db_uuid = items.db_uuid ",
                                                      " AND links.session = items.session ",
                                                      " AND links.bundle = items.bundle ",
                                                      " AND links.from_id = items.item_id",
                                                      " AND items.level = '", connectHierPath[i - 1], "' ", # limit to parent level
                                                      " AND items.session REGEXP '", sessionPattern, "' ", # limit to session RegEx
                                                      " AND items.bundle REGEXP '", bundlePattern, "' ", # limit to bundle RegEx
                                                      ""))
        ...
        DBI::dbExecute(emuDBhandle$connection, paste0("INSERT INTO hier_right_trapeze_interm_res_tmp ",
                                                      ...
                                                      " AND lstn.session = links.session ",
                                                      " AND lstn.bundle = links.bundle ",
                                                      " AND lstn.seq_end_id = links.to_id ", # right side of seq
                                                      " AND links.db_uuid = items.db_uuid ",
                                                      " AND links.session = items.session ",
                                                      " AND links.bundle = items.bundle ",
                                                      " AND links.from_id = items.item_id",
        ...
        leftTrapezeTmp = DBI::dbGetQuery(emuDBhandle$connection, paste0("SELECT DISTINCT ",
                                                                        ...
                                                                        " items.seq_idx AS seq_start_seq_idx, ",
                                                                        " items.seq_idx AS seq_end_seq_idx, ",
                                                                        " hltirt.db_uuid_child, ",
                                                                        " hltirt.session_child, ",
                                                                        " hltirt.bundle_child, ",
                                                                        " hltirt.seq_start_id_child, ",
                                                                        " hltirt.seq_end_id_child, ",
                                                                        " hltirt.seq_len_child, ",
                                                                        " hltirt.level_child, ",
                                                                        " hltirt.seq_start_seq_idx_child, ",
                                                                        " hltirt.seq_end_seq_idx_child ",
                                                                        "FROM hier_left_trapeze_interm_res_tmp AS hltirt, ",
                                                                        " links, ",
                                                                        " items ",
                                                                        "WHERE hltirt.db_uuid = links.db_uuid ",
                                                                        " AND hltirt.session = links.session ",
                                                                        " AND hltirt.bundle = links.bundle ",
                                                                        " AND hltirt.seq_start_id = links.to_id ", # left side of seq
                                                                        " AND links.db_uuid = items.db_uuid ",
                                                                        " AND links.session = items.session ",
                                                                        " AND links.bundle = items.bundle ",
                                                                        " AND links.from_id = items.item_id", 
        ...
        rightTrapezeTmp = DBI::dbGetQuery(emuDBhandle$connection, paste0("SELECT DISTINCT ",
                                                                         ...
                                                                         " items.seq_idx AS seq_start_seq_idx, ",
                                                                         " items.seq_idx AS seq_end_seq_idx, ",
                                                                         " hrtirt.db_uuid_child, ",
                                                                         " hrtirt.session_child, ",
                                                                         " hrtirt.bundle_child, ",
                                                                         " hrtirt.seq_start_id_child, ",
                                                                         " hrtirt.seq_end_id_child, ",
                                                                         " hrtirt.seq_len_child, ",
                                                                         " hrtirt.level_child, ",
                                                                         " hrtirt.seq_start_seq_idx_child, ",
                                                                         " hrtirt.seq_end_seq_idx_child ",
                                                                         "FROM hier_right_trapeze_interm_res_tmp AS hrtirt, ",
                                                                         " links, ",
                                                                         " items ",
                                                                         "WHERE hrtirt.db_uuid = links.db_uuid ",
                                                                         " AND hrtirt.session = links.session ",
                                                                         " AND hrtirt.bundle = links.bundle ",
                                                                         " AND hrtirt.seq_end_id = links.to_id ", # right side of seq
                                                                         " AND links.db_uuid = items.db_uuid ",
                                                                         " AND links.session = items.session ",
                                                                         " AND links.bundle = items.bundle ",
                                                                         " AND links.from_id = items.item_id ",
        ...
        # extract child and parent values persisting the left and right sides as the input tables
        ...
        if(leftIsChild){
          ...
          View(DBI::dbGetQuery(emuDBhandle$connection, paste0(#"INSERT INTO lr_exp_res_tmp ",
            ...
            # " pstn.session, ",
            # " pstn.bundle, ",
            # " cstn.seq_start_id AS l_seq_start_id, ",
            # " cstn.seq_end_id AS l_seq_end_id, ",
            # " cstn.seq_len AS l_seq_len, ",
            # " cstn.level AS l_level, ",
            # " cstn.seq_start_seq_idx AS l_seq_start_seq_idx, ",
            # " cstn.seq_end_seq_idx AS l_seq_end_seq_idx,",
            # " pstn.seq_start_id AS l_seq_start_id, ",
            # " pstn.seq_end_id AS l_seq_end_id, ",
            # " pstn.seq_len AS r_seq_len, ",
            # " pstn.level AS r_level, ",
            # " pstn.seq_start_seq_idx AS r_seq_start_seq_idx, ",
            # " pstn.seq_end_seq_idx AS r_seq_end_seq_idx ",
            "FROM ", parentSideTableName , " AS pstn, ",
            " hier_left_trapeze_interm_res_tmp AS hltirt, ",
            " hier_right_trapeze_interm_res_tmp AS hrtirt, ",
            childSideTableName, " AS cstn_start_seq ",
            childSideTableName, " AS cstn_end_seq ",
            "WHERE pstn.db_uuid = hltirt.db_uuid ",
            " AND pstn.session = hltirt.session ",
            " AND pstn.bundle = hltirt.bundle ",
            " AND pstn.seq_start_id = hltirt.seq_start_id ",
            " AND pstn.db_uuid = hrtirt.db_uuid ",
            " AND pstn.session = hrtirt.session ",
            " AND pstn.bundle = hrtirt.bundle ",
            " AND pstn.seq_end_id = hrtirt.seq_end_id ",
            " AND cstn_start_seq.db_uuid = hltirt.db_uuid ",
            " AND cstn_start_seq.session = hltirt.session ",
            " AND cstn_start_seq.bundle = hltirt.bundle ",
            " AND cstn_start_seq.seq_start_id = hltirt.seq_start_id_child ",
            " AND cstn.db_uuid = hrtirt.db_uuid ",
          ...
          #                                               "SET l_seq_start_id = joined.item_id ",
          #                                               "FROM ( ",
          ...
          #                                               " AND lr_exp_res_tmp.l_level = items.level ",
          #                                               " AND lr_exp_res_tmp.l_seq_start_seq_idx = items.seq_idx ",
          #                                               ") as joined ",
          ...
          #                                               "AND lr_exp_res_tmp.l_level = joined.level ",
          #                                               "AND lr_exp_res_tmp.l_seq_start_seq_idx = joined.seq_idx ",
          #                                               ""))
          
          # DBI::dbExecute(emuDBhandle$connection, paste0("UPDATE lr_exp_res_tmp ",
          #                                               "SET l_seq_end_id = joined.item_id ",
          #                                               "FROM ( ",
          #                                               " SELECT items.item_id AS item_id, ",
          #                                               "  items.db_uuid, ",
          #                                               "  items.session, ",
          ...
          #                                               " AND lr_exp_res_tmp.bundle = items.bundle ",
          #                                               " AND lr_exp_res_tmp.l_level = items.level ",
          #                                               " AND lr_exp_res_tmp.l_seq_end_seq_idx = items.seq_idx ",
          #                                               ") as joined ",
          #                                               "WHERE lr_exp_res_tmp.db_uuid = joined.db_uuid ",
          #                                               "AND lr_exp_res_tmp.session = joined.session ",
          #                                               "AND lr_exp_res_tmp.bundle = joined.bundle ",
          #                                               "AND lr_exp_res_tmp.l_level = joined.level ",
          #                                               "AND lr_exp_res_tmp.l_seq_end_seq_idx = joined.seq_idx ",
          #                                               ""))
          ...
          DBI::dbExecute(emuDBhandle$connection, paste0("INSERT INTO lr_exp_res_tmp ",
                                                        ...
                                                        " pstn.seq_start_id AS l_seq_start_id, ",
                                                        " pstn.seq_end_id AS l_seq_end_id, ",
                                                        " pstn.seq_len AS l_seq_len, ",
                                                        " pstn.level AS l_level, ",
                                                        " pstn.seq_start_seq_idx AS l_seq_start_seq_idx, ",
                                                        " pstn.seq_end_seq_idx AS l_seq_end_seq_idx, ",
                                                        " cstn.seq_start_id AS r_seq_start_id, ",
                                                        " cstn.seq_end_id AS r_seq_end_id, ",
                                                        " cstn.seq_len AS r_seq_len, ",
                                                        " cstn.level AS r_level, ",
                                                        " cstn.seq_start_seq_idx AS r_seq_start_seq_idx, ",
                                                        " cstn.seq_end_seq_idx AS r_seq_end_seq_idx ",
                                                        "FROM ", parentSideTableName , " AS pstn, ",
                                                        " hier_left_trapeze_interm_res_tmp AS hltirt, ",
                                                        " hier_right_trapeze_interm_res_tmp AS hrtirt, ",
                                                        childSideTableName, " AS cstn ",
                                                        "WHERE pstn.db_uuid = hltirt.db_uuid ",
                                                        " AND pstn.session = hltirt.session ",
                                                        " AND pstn.bundle = hltirt.bundle ",
                                                        " AND pstn.seq_start_id = hltirt.seq_start_id ",
                                                        ...
                                                        " AND pstn.bundle = hrtirt.bundle ",
                                                        " AND pstn.seq_end_id = hrtirt.seq_end_id ",
                                                        " AND cstn.db_uuid = hltirt.db_uuid ",
                                                        " AND cstn.session = hltirt.session ",
                                                        " AND cstn.bundle = hltirt.bundle ",
                                                        " AND cstn.seq_start_id = hltirt.seq_start_id_child ",
                                                        " AND cstn.db_uuid = hrtirt.db_uuid ",
                                                        " AND cstn.session = hrtirt.session ",
                                                        " AND cstn.bundle = hrtirt.bundle ",
                                                        " AND cstn.seq_end_id = hrtirt.seq_end_id_child ",
  ...
  # insert NA rows if preserveChildLength or preserveParentLength are set
  if(preserveChildLength){
    if(leftIsChild){
      preservedLengthTable = DBI::dbGetQuery(emuDBhandle$connection, paste0("SELECT lr_exp_res_tmp.* ",
                                                                            "FROM ", childSideTableName, " AS cstn ",
                                                                            "LEFT JOIN lr_exp_res_tmp ",
                                                                            "ON cstn.db_uuid = lr_exp_res_tmp.db_uuid ",
                                                                            " AND cstn.session = lr_exp_res_tmp.session ",
                                                                            " AND cstn.bundle = lr_exp_res_tmp.bundle ",
                                                                            " AND cstn.seq_start_id = lr_exp_res_tmp.l_seq_start_id ",
                                                                            " AND cstn.seq_end_id = lr_exp_res_tmp.l_seq_end_id ",
                                                                            "ORDER BY cstn.rowid ",
                                                                            ""))
    } else {
      preservedLengthTable = DBI::dbGetQuery(emuDBhandle$connection, paste0("SELECT lr_exp_res_tmp.* ",
                                                                            ...
                                                                            " AND cstn.session = lr_exp_res_tmp.session ",
                                                                            " AND cstn.bundle = lr_exp_res_tmp.bundle ",
                                                                            " AND cstn.seq_start_id = lr_exp_res_tmp.r_seq_start_id ",
                                                                            " AND cstn.seq_end_id = lr_exp_res_tmp.r_seq_end_id ",
                                                                            "ORDER BY cstn.rowid ",
                                                                            ""))
    }
  ...
  # if(preserveChildLength | preserveParentLength){
  #   
  #   # set table name and seq_*_id prefix used for collapsing
  #   # e.g.: if preserveChildLength & leftIsChild -> use left side for grouping (hence collapsing)
  #   if(preserveChildLength){
  #     preserveTableName = childSideTableName
  #     if(leftIsChild){ 
  #       seqIdPrefix = "l_"
  #     }else{
  #       seqIdPrefix = "r_"
  #     }
  #   }else{
  #     preserveTableName = parentSideTableName
  #     if(leftIsChild){ 
  #       seqIdPrefix = "r_"
  #     }else{
  #       seqIdPrefix = "l_"
  ...
  # this avoids expansion in e.g. MANY_TO_MANY relationships
  ...
  #                                                                       "FROM ", preserveTableName, " AS pstn ",
  #                                                                       "LEFT JOIN lr_exp_res_collapsed_tmp AS lr ",
  #                                                                       "ON pstn.db_uuid = lr.db_uuid ",
  #                                                                       " AND pstn.session = lr.session ",
  #                                                                       " AND pstn.bundle = lr.bundle ",
  #                                                                       " AND pstn.seq_start_id = lr.", seqIdPrefix, "seq_start_id ",
  #                                                                       " AND pstn.seq_end_id = lr.", seqIdPrefix, "seq_end_id ",
  ...
  #                                                                       "GROUP BY pstn.rowid ", # preserve pstn length
  #                                                                       "ORDER BY pstn.rowid ", # preserve pstn ordering
  #                                                                       ""))
  # 
  # # drop temp table
