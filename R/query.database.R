...
# ##' 
# ##' }
# contextQuery<-function(seglist, startOffset=0,seqLength=1,resultType=NULL){
#   if(!inherits(seglist,"emuR.segmentList")){
#     stop("Segment list 'seglist' must be of type 'emuR.segmentList'. (Do not set a value for 'resultType' parameter for the query, the default resultType wiil be used)")
#   }
#   
#   distinctEmuDbs=sqldf("SELECT DISTINCT db_uuid FROM seglist")
#   distinctEmuDbsCnt=nrwo(distinctEmuDbs)
#   
...
#   }
#   dbUUID=distinctEmuDbsCnt[1,'db_uuid']
#   #items=dbReadTable()
#   
#   itemsIdxSql='CREATE INDEX items_idx ON items(itemID,db_uuid,session,bundle,level,itemID,seqIdx,type,sampleRate,sampleStart,sampleDur,samplePoint)'
#   resIdxSql='CREATE INDEX its_idx ON its(db_uuid,session,bundle,seqStartId,seqEndId,seqLen,level)'
