                                       ...
                                       verbose)
{
  ...
  if (nrow(bundles_list) > 0)
  {
    for (bundle_idx in 1:nrow(bundles_list))
    {
      ...
      queryTxt = paste0(
        "SELECT label, item_id FROM ", BAS_LABELS_TMP_TABLE, basic_cond(handle, session, bundle),
        "AND name=='", canoLabel, "'"
      )
...
<<<<<<< HEAD
        ...
        if (!is.null(trnLevel))
        {
          ...
          queryTxt = paste0(
            "SELECT item_id, sample_start, sample_dur FROM ", BAS_ITEMS_TMP_TABLE, basic_cond(handle, session, bundle),
            "AND level=='", trnLevel, "'"
          )
...
<<<<<<< HEAD
              ...
              write(paste(
                "TRN:", turn_start, turn_duration, paste0(linkstrings, collapse = ","), "<NA>\n"
              ), kancom)
=======
...
<<<<<<< HEAD
        res = RCurl::postForm(
          address, LANGUAGE = language, OUTFORMAT = "par", USETRN = usetrn,
...
<<<<<<< HEAD
              ...
              if (as.integer(bas_id) >= 0)
              {
                bas_add_item(
                  ...
                  seq_idx = seq_idx, item_id = item_id, level =
                    mauLevel, samplerate = samplerate, type = "SEGMENT",
                  sample_start = start, sample_dur = duration
                )
                ...
                bas_add_label(
                  handle = handle, session = session, bundle = bundle,
                  item_id = item_id, label_idx = 1, label_name =
                    mauLabel,
                  label = label
                )
                ...
                bas_add_link(
                  handle = handle, session = session, bundle = bundle,
                  from_id = bas_id_to_item_id[[splitline[4]]], to_id =
                    item_id
                )
              }
=======
...
<<<<<<< HEAD
      res = RCurl::postForm(
        address, LANGUAGE = language, OUTFORMAT = "mau",
        SIGNAL = RCurl::fileUpload(signalfile), style = "HTTPPOST"
      )
        ...
        for (line_idx in 1:length(minniLines))
        {
          ...
          if (stringr::str_detect(line, "^MAU:"))
          {
            ...
            if (as.integer(bas_id) >= 0)
            {
              bas_add_item(
                handle = handle, session = session, bundle = bundle,
                seq_idx = seq_idx, item_id = item_id, level = minniLevel, samplerate =
                  samplerate, type = "SEGMENT",
                sample_start = as.integer(splitline[2]), sample_dur =
...
<<<<<<< HEAD
            res = RCurl::postForm(
              address, lng = language, iform = "txt", nrm = "yes",
              ...
              for (line_idx in 1:length(g2pLines))
              {
                ...
                if (stringr::str_detect(line, "^ORT:"))
                {
                  ...
                  bas_add_item(
                    handle = handle, session = session, bundle = bundle,
                    seq_idx = seq_idx, item_id = item_id, level =
                      orthoLevel, samplerate = samplerate, type = "ITEM"
                  )
                  ...
                  bas_add_label(
                    handle = handle, session = session, bundle = bundle,
                    item_id = item_id, label_idx = 1, label_name =
                      orthoLabel, label = label
                  )
                  
                  bas_add_link(
                    handle = handle, session = session, bundle = bundle,
                    from_id = transcription_item_id, to_id =
                      item_id
                  )
=======
...
<<<<<<< HEAD
        ...
        address = paste0(BAS_ADDRESS, "runG2P")
        res = RCurl::postForm(
          address, lng = language, iform = "bpf",
        ...
        if (length(g2pLines) > 0)
        {
          for (line_idx in 1:length(g2pLines))
          {
            ...
            if (stringr::str_detect(line, "^KAN:"))
            {
              ...
              label = str_replace_all(splitline[3], "'", "''")
              queryTxt = paste0(
                "SELECT max(label_idx) FROM ", BAS_LABELS_TMP_TABLE, basic_cond(handle, session, bundle),
                "AND item_id==", item_id
              )
              ...
              bas_add_label(
                handle = handle, session = session, bundle = bundle,
                item_id = item_id, label_idx = label_idx, label_name =
                  canoLabel, label = label
              )
=======
        address = paste0(BAS_ADDRESS, "runG2P")
...
<<<<<<< HEAD
  queryTxt = paste0(
    "CREATE TEMP TABLE ", BAS_ITEMS_TMP_TABLE, " (",
    "db_uuid VARCHAR(36),",
    "session TEXT,",
    "bundle TEXT,",
    "item_id INTEGER,",
  ...
  queryTxt = paste0(
    "CREATE TEMP TABLE ", BAS_LINKS_TMP_TABLE, " (",
    "db_uuid VARCHAR(36),",
    "session TEXT,",
    "bundle TEXT,",
    "from_id INTEGER,",
  ...
  queryTxt = paste0(
    "CREATE TEMP TABLE ", BAS_LABELS_TMP_TABLE, " (",
    "db_uuid VARCHAR(36),",
    "session TEXT,",
    "bundle TEXT,",
    "item_id INTEGER,",
...
<<<<<<< HEAD
  ...
  queryTxt = paste0("INSERT OR IGNORE INTO labels SELECT * FROM ", BAS_LABELS_TMP_TABLE)
  DBI::dbGetQuery(handle$connection, queryTxt)
=======
...
<<<<<<< HEAD
  download.file(
    downloadLink, target, method = "auto", quiet = T,
    mode = "w", cacheOK = TRUE
  )
...
{
<<<<<<< HEAD
  queryTxt = paste0("SELECT max(item_id) FROM ", BAS_ITEMS_TMP_TABLE, basic_cond(handle, session, bundle))
...
{
  queryTxt = paste0(
    "INSERT INTO ", BAS_ITEMS_TMP_TABLE, " VALUES('", handle$UUID,
    "','", session, "','", bundle,
    "',", item_id, ",'", level, "','", type, "', ",
    seq_idx, ",", samplerate, ",", sample_point, ",",
    sample_start, ",", sample_dur, ")"
  )
  DBI::dbGetQuery(handle$connection, queryTxt)
}
...
{
  queryTxt = paste0(
    "INSERT INTO ", BAS_LABELS_TMP_TABLE, " VALUES('", handle$UUID,
    "', '", session, "', '", bundle,
    "', ", item_id, ", ", label_idx,
    ", '", label_name, "', '", label, "')"
  )
...
{
  queryTxt = paste0(
    "INSERT INTO ", BAS_LINKS_TMP_TABLE, " VALUES('", handle$UUID,
    "', '", session, "', '", bundle,
    "', ", from_id, ", ", to_id,
    ", NULL)"
  )
...
<<<<<<< HEAD
  queryTxt = paste0(
    "SELECT annotates FROM bundle", basic_cond(handle, session, bundle, bundlename =
                                                 "name")
  )
  ...
  {
    return(file.path(
      handle$basePath, paste0(session, session.suffix), paste0(bundle, bundle.dir.suffix), res[1,1]
    ))
=======
...
<<<<<<< HEAD
  queryTxt = paste0(
    "SELECT sample_rate FROM bundle", basic_cond(handle, session, bundle, bundlename =
                                                   "name")
  )
  res = dbGetQuery(handle$connection, queryTxt)
  if (nrow(res) > 0)
...
<<<<<<< HEAD
  queryTxt = paste0(
    "SELECT item_id FROM ", BAS_ITEMS_TMP_TABLE, basic_cond(handle, session, bundle), "AND level=='", topLevel, "'"
  )
  ...
  if (nrow(res) > 1) {
    stop("More than one possible node on level ", topLevel, " in bundle ", bundle)
  }
  return(res[[1,1]])
}
...
bas_new_canvas <- function(handle, perspective, canvas)
{
  if (!is.null(perspective))
  {
    ...
    set_levelCanvasesOrder(handle, perspectiveName = perspective,
