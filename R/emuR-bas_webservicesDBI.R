...
bas_run_text_align <- function(handle,
                               superLabel,
                               subLabel,
                               superLevel,
                               subLevel,
                               ...
                               verbose)
{
  bundles_list = list_bundlesDBI(handle)
  if (nrow(bundles_list) > 0)
  {
    bas_ping(verbose)
    if (verbose)
    ...
    queryTxt = paste0("[", superLabel,"=~.*\\S.*]")
    super_items = query(handle, queryTxt, calcTimes = T)
    
    queryTxt = paste0("[", subLabel,"=~.*\\S.*]")
    sub_items = query(handle, queryTxt, calcTimes = T)
    ...
    for (bundle_idx in 1:nrow(bundles_list))
    {
      ...
      if (resume &&
          bas_link_exists_in_bundle(handle, session, bundle, superLevel, subLevel))
      {
        if (verbose)
        {
          cat(" Skipping bundle", bundle)
      ...
      super_items_bundle = super_items[super_items$bundle == bundle &
                                       super_items$session == session,]
      
      sub_items_bundle = sub_items[sub_items$bundle == bundle &
                                     sub_items$session == session,]
      
      
      if (nrow(super_items_bundle) > 0 && nrow(sub_items_bundle) > 0)
      {

        infile = file.path(BAS_WORKDIR, paste0(bundle, ".in.csv"))
        outfile = file.path(BAS_WORKDIR, paste0(bundle, ".out.csv"))
        
        csvcon <- file(infile)
        open(csvcon, "w")
        
        item_ids_super = super_items_bundle$start_item_id
        item_ids_sub = sub_items_bundle$start_item_id
        
        write(paste0(super_items_bundle$labels, collapse = " "), csvcon)
        write(";", csvcon)
        write(paste0(sub_items_bundle$labels, collapse = " "), csvcon)
        
        close(csvcon)
        
        curlParams = list(
          i = RCurl::fileUpload(infile),
          atype = "dir"
        )
        ...
        res = RCurl::postForm(
          paste0(BAS_ADDRESS, "runTextAlign"),
          .params = curlParams,
          style = "HTTPPOST",
          .opts = bas_get_options()
        )
        
        zipfile = bas_download(res, outfile)
        utils::unzip(zipfile, exdir = file.path(BAS_WORKDIR, "textalign"))
        
        list.dirs(BAS_WORKDIR, "textalign")
        ...
        results = stringr::str_split(stringr::str_trim(csvLines[1]), ";")
        list1 = stringr::str_split(results[1], " ")
        list2 = stringr::str_split(results[2], " ")
        
        for(idx in 1:length(list1))
        {
          if(list2[idx] != "_")
          {
            bas_add_link(
